...
def getdata(file):
    ...
    for i in range(int(num)):#读取本体
        ...
        data = data.replace("带盖板", "[保留_盖板]").replace("不带盖板", "[保留_不盖板]").replace("扎带", "[扎]")
        # 删除“配”、“带”、“用于”开头的中文或英文（但保留数字、符号）
        # data = re.sub(r'(配|带|用于)[\u4e00-\u9fa5A-Za-z]*', '', data)
        data = re.sub(r'配.*', '', data)
        data = re.sub(r'带.*', '', data)
        data = re.sub(r'用于.*', '', data)
        # 把占位符还原
        data = data.replace("[保留_盖板]", "带盖板").replace("[保留_不盖板]", "不带盖板").replace("[扎]", "扎带")
        # data = re.sub(r'配.*?安装附件', '', data)
        data = data + ',' + tibian + ',' + bihou
        print(data)
        a = re.sub(u"\\（.*?）|\\{.*?}|\\[.*?]|\\【.*?】", "", data)
        data = re.sub(u"\\(.*?\\)|\\{.*?}|\\[.*?]", "", a)
        datas.append(data)
...
def data_dealer(datas):
    ...
    for val in datas:
        ...
        if (val.find('直通') != -1 or val.find('弯通') != -1 or val.find('弯头') != -1 or val.find('三通') != -1 or val.find('四通') != -1 or val.find('凹') != -1 or val.find('异径接头') != -1 or val.find('凸') != -1 or val.find('盖板') != -1) and val.find('抱箍') == -1 and val.find('封头') == -1 and val.find('管接头') == -1 and val.find('扎带') == -1 and val.find('隔板') == -1 and val.find('直接片') == -1 and val.find('连接板') == -1 and val.find('弯接片') == -1 and val.find('调角片') == -1 and val.find('调宽片') == -1 and val.find('调高片') == -1 and val.find('连接线') == -1 and val.find('接地线') == -1 and val.find('跨接线') == -1 and val.find('接连线') == -1 and val.find('固定压板') == -1 and val.find('胶垫') == -1 and val.find('配套') == -1 and val.find('用于') == -1:
            val = re.sub(r'X', '×', val)
            val = re.sub(r'x', '×', val)
            val = re.sub(r'\*', '×', val)
            ...
            elif val.find('封头') != -1 or val.find('端头') != -1:
                classes = 'fengtou'
                features = fujian.fengtou(vals)
            elif val.find('管接头') != -1 or val.find("穿板接头") != -1:
                classes = 'guanjietou'
...
def itself(val,wh):
    ...
    for data in val[:-2]:
        ...
        if data.find('直通') != -1:
            ...
            classes = '水平弯通'
        elif data.find('垂直') != -1 or data.find('凹') != -1 or data.find('凸') != -1 or data.find('异径接头') != -1 or data.find('上') != -1 or data.find('下') != -1 or data.find('高低')!=-1 or data.find('大小头')!=-1 or data.find('异径弯头')!=-1 or data.find('异径弯通')!=-1 or data.find('异径直通')!=-1 or data.find('异径槽体')!=-1 :
            classes = '垂直弯通'
        if data.find('盖板') != -1 or data.find('护罩') != -1:
            if (data.find('配') != -1 or data.find('有') != -1 or data.find('带') != -1) and data.find('不带盖板') == -1:
                have_gaiban = 'yes'
            elif data.find('非盖板') != -1 or data.find('无盖板') != -1 or data.find('非护罩') != -1 or data.find('无护罩') != -1 or data.find('不带盖板') != -1:
                have_gaiban = 'no'
            else:
                zhonglei = 'gaiban'
...
def getprice(base, data, file, _):
    ...
    for val in data:
        if val[-2] == 'qiaojia':
            ...
            price, h = get_p.luoshuan(val, sheet)
            if price == 0:
                prices = ['错误', '错误']
            else:
                prices = [price, h]
