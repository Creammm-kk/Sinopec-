...
def qiaojia(val,sheet,rank): #rank从1开始
    ...
    for m in range(sheet.min_row,sheet.max_row+1):
        ...
        # print(cell1)
        if cell1 != None and cell1.find(whh) != -1 and cell1.find(whh+'.5') == -1 and cell1.find(val[-1]) != -1 and cell1.find(val[-3])!=-1 and cell1.find(val[-3])!=-1 and cell1.find(fpen)==-1:
            if val[2] == 'tishi':
                if cell1.find('梯式') != -1:
                    flag = 1
                else:
...
def fengtou(val, sheet, rank):
    ...
    for m in range(sheet.min_row, sheet.max_row + 1):
        ...
        cell1 = sheet.cell(m, 5).value
        # cell2 = sheet.cell(m, 2).value
        if cell1 != None and cell1.find(whh)!=-1 and cell1.find(whh+".5")==-1 and (cell1.find('封头')!=-1 or cell1.find('终端头')!=-1) and cell1.find(val[-3])!=-1 and cell1.find(fpen)==-1:
            price += sheet.cell(m, rank+6).value
            break
    return price, h
def gaiban(val, sheet, rank):
    ...
    if val[-1]=='直通桥架':
        name = '直通'
    elif val[-1] == '水平三通' or val[-1] == '垂直三通':
        name = '三通'
    elif val[-1] == '水平四通' or val[-1] == '垂直四通':
        name = '四通'
    else:
        name = val[-1]
    if bihou % 1 == 0:
    ...
    for m in range(sheet.min_row, sheet.max_row + 1):
        ...
        # print(cell1)
        # cell2 = sheet.cell(m, 2).value
        if cell1 != None and cell1.find(whh)!=-1 and cell1.find(whh+".5")==-1 and cell1.find('盖板')!=-1 and cell1.find('带盖板')==-1 and cell1.find('无盖板')==-1 and cell1.find(name)!=-1 and cell1.find(val[-3])!=-1 and cell1.find(fpen)==-1:
...
def geban(val, sheet, rank):
    ...
    for m in range(sheet.min_row, sheet.max_row + 1):
        ...
        print(height+'×'+houdu)
        if houdu.find(".0") != -1:
            houdu1 = houdu.replace('.0', '')
        else:
            houdu1 = houdu
        if cell1 != None and (cell1.find(height+'×'+houdu)!=-1 or cell1.find(height+'×'+houdu1)!=-1) and cell1.find(height+'×'+houdu+'.5')==-1 and cell1.find(name)!=-1 and cell1.find(val[-3])!=-1 and cell1.find(fpen)==-1:
            price += sheet.cell(m, rank+6).value
            break
    return price, h
