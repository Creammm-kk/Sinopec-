...
def getdata(file):
    ...
    for val in datas:
        ...
        for x in val:
            ...
            if flag != 1 and [x.find('支架') != -1 or x.find('护栏') != -1 or x.find('吸顶') != -1 or x.find('壁') != -1 or x.find('座') != -1 or x.find('法兰') != -1 or x.find('吊杆') != -1 or x.find('弯杆') != -1 or [[x.find('米') != -1 or x.find('m') != -1 or x.find('M') != -1] and name == '路灯']]:
                fangshi = x
                if name.find("航空") != -1:
                    if fangshi.find('壁') != -1:
                        fangshi1 = '壁式'
                ...
                if fangshi.find('支架') != -1:
                    ...
                    if fangshi.find('米') != -1 or fangshi.find('m') != -1 or fangshi.find('M') != -1:
...
def deal_cell(cell):
    cell = re.sub(r'（|）|/(|/)|LED|防爆|高效|≥|≤| |双管|单管|高度|全塑|W|w', '', cell)
    return cell
...
def getprice(base,data,file,_):
    ...
    for d in data:
        ...
        for m in range(sheet.min_row,sheet.max_row+1):
            ...
            cell_sp = cell.split("×")
            if len(cell_sp)==2:
                cell = cell_sp[0] + '×' + cell_sp[1]
                cell_change = cell_sp[1] + '×' + cell_sp[0]
            else:
                cell_change = cell
            if cell == deal_cell(d[1]) or cell_change == deal_cell(d[1]):
                print('计价表,', cell, cell1, cell2)
                print('我的总结', d)
                if cell1 == d[0] and cell2 == d[2]:
                    price = sheet.cell(h,9).value
                    price_list.append(h)
